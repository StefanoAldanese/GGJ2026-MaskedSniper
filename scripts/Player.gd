...
func _process_panic_mode(delta):
	...
	# Il timer conta silenziosamente
	panic_timer -= delta
	
	# LE LANCETTE GIRANO VELOCI (EFFETTO VISIVO)
	if clock_hand_long:
		# 3 giri al secondo
...
# --- LOGICA NORMALE (TRACCIA IL TEMPO) ---
...
# Funzione per posizionare le lancette in base al tempo (usata solo in Normal Mode ora)
